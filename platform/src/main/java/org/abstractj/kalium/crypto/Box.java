...
public class Box {
    ...
    public Box(byte[] publicKey, byte[] privateKey) {
        ...
        checkLength(publicKey, SodiumConstants.PUBLICKEY_BYTES);
        checkLength(privateKey, SodiumConstants.SECRETKEY_BYTES);
    }
    ...
    public byte[] encrypt(byte[] nonce, byte[] message) {
        checkLength(nonce, SodiumConstants.NONCE_BYTES);
        byte[] msg = prependZeros(SodiumConstants.ZERO_BYTES, message);
        ...
        return removeZeros(SodiumConstants.BOXZERO_BYTES, ct);
    }
    ...
    public byte[] decrypt(byte[] nonce, byte[] ciphertext) {
        checkLength(nonce, SodiumConstants.NONCE_BYTES);
        byte[] ct = prependZeros(SodiumConstants.BOXZERO_BYTES, ciphertext);
        ...
        return removeZeros(SodiumConstants.ZERO_BYTES, message);
    }
